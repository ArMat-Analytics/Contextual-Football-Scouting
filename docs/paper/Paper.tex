...
and (H4)~\textbf{Player Similarity}, a within-role similarity model that compares players by their contextual ``spatial DNA'' and surfaces cost-effective alternatives.
...
Combined with the Transfermarkt market values from \cref{sec:transfermarkt}, this operationalises the value-arbitrage use case: for a premium target, the model ranks his cheaper stylistic look-alikes by \textbf{pre-Euro} price (the value \emph{before} the tournament revealed his quality), with the post-Euro value retained only to back-test whether the cheap match's price subsequently rose.
