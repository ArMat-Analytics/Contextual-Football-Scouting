...
\title{%
    \textbf{Contextual Football Scouting}\\[4pt]
...
% ============================================================
\begin{abstract}
Traditional scouting statistics reward volume (passes played, distance covered, and touches taken) and in doing so carry a heavy \textbf{team bias}: a midfielder in a possession-dominant side accumulates numbers that a better player in a pressed side never gets the chance to produce.
This project reads players from \textbf{StatsBomb~360 spatial data} rather than from the event log alone, and quantifies and demonstrates the \textbf{spatial intelligence} of a player. By analyzing a team and its opponent positioning during specific actions, it becomes possible to distinguish between players who genuinely excel under pressure and those who merely benefit from a favorable tactical framework.
We present four completed studies, each with its own metric, notebook and validation, unified by two design choices: \textbf{Expected Possession Value~(EPV)} as the common currency, and \textbf{within-role percentiles} as the common lens (e.g. a centre-back is ranked only against other centre-backs).
The four metrics are:
(H1)~\textbf{Space Control~\&~Value}, a four-index spatial-influence profile;
(H2)~\textbf{Decision Quality}, the value of the pass a player chose relative to the in-frame options he ignored;
(H3)~\textbf{Off-Ball Movement}, the dangerous off-ball space a player occupies that his teammates fail to use;
and (H4)~\textbf{Player Similarity}, a similarity model that groups players by their contextual ``spatial DNA'' and surfaces cost-effective alternatives.
All four run on the same 272-player pool from UEFA Euro~2024.
The four studies feed an interactive web dashboard built as a contextual scouting tool.
We show that ball volume is a poor proxy for value created: on the EPV-creation index, more than half the pool shifts by $20+$~percentile points when moving from a volume ranking to a contextual one. Furthermore, the off-ball metric isolates a real and previously invisible signal: the ``shadow runner'' who keeps offering dangerous options his team does not use.
\end{abstract}
...
This matters because most clubs still rely heavily on decontextualised \textbf{event data}, such as aggregate passes, goals, and touches.
...
The thesis of this project is that a player's quality can be read from \textbf{what he does with the space around him}, measured by the \textbf{value} of those actions rather than their volume, and that doing so removes a large part of the team-bias inflation that distorts traditional comparisons.

A note on method, because it shaped everything that follows.
The hard part of this project was \textbf{engineering metrics that are football-true}: that a non-technical staff member would recognise as describing a real on-pitch behaviour.
...
\toprule
& \textbf{Hypothesis} & \textbf{What it measures} & \textbf{Headline metric} \\
\midrule
\textbf{H1} & Space Control \& Value &
...
\begin{enumerate}
    \item A \textbf{contextual, team-bias-corrected scouting framework} that grades players by EPV value, not event volume, and benchmarks every player within his macro-role rather than against the whole pitch.
    \item \textbf{Four novel, independently validated metrics}: Space Control, Decision Quality, Off-Ball Movement, and Player Similarity, built entirely from 360 freeze-frame spatial data.
    \item A \textbf{receiver resolver} that puts identities on the anonymous off-ball dots of a 360~frame, validated at \textbf{93\% accuracy} against ground-truth pass recipients, which is what makes per-player off-ball rating possible at all.
    \item An \textbf{interactive scouting platform} (web dashboard) that presents the metric families side by side on a single player card, with within-role head-to-head comparison and a similarity-powered player search.
\end{enumerate}
...
The analysis is built on \textbf{StatsBomb Open Data}, specifically the \textbf{360~frames} for \textbf{UEFA Euro~2024} (\texttt{competition\_id\,=\,55}, \texttt{season\_id\,=\,282}), covering \textbf{51~matches}.
...
The analysis pool is the \textbf{272~players} who clear a \textbf{135-minute floor} (about 1.5~matches), with goalkeepers excluded.
...
A critical property of a 360~frame, central to H3, is that it gives positions \textbf{but no identities}: only the player on the ball is named; every other visible player carries just a team tag.
...
To read on-pitch performance against financial cost (the value-arbitrage angle of the business case), we scrape \textbf{market values} from \textbf{Transfermarkt} for the Euro~2024 participants.
...
It is the single currency through which every metric grades a player by the \textbf{value} of what he does.
...
EPV is used directly in H1 (the value added by each progression) and, in its predictive form \textbf{xEPV}, in both H2 and H3 (the expected net value of a pass that \emph{could} be played).
...
Every headline and every radar axis, in all metrics, is a player's \textbf{percentile rank inside his macro-role}, not against the whole pitch.
The macro-roles are \textbf{CB} (centre-back), \textbf{FB} (full-back), \textbf{MID} (central midfielder), \textbf{CAM} (attacking midfielder), \textbf{WIDE} (wide attacker) and \textbf{FW} (forward).
...
The clearest example: the median share of touches taken \emph{inside} the opposing block is about \textbf{4\%} for centre-backs against about \textbf{52\%} for attacking midfielders.
...
The macro-role is assigned \textbf{once per player}, as the \textbf{modal role across all his events} (i.e., the role he actually played most, not his nominal line-up label).
...
The assignment is computed in H1 and written to one file; H2, H3 and H4 read that exact same file, so the 272-player role assignment is \textbf{byte-identical} across all four studies and a player's profiles can be placed side by side without any re-matching.
...
All four metrics run on the \textbf{identical 272-player pool} ($\geq 135$~minutes, no goalkeepers), and read the same per-player macro-role from a single shared file.
...
The H1 player-totals pipeline first keeps all \textbf{357~players} with $\geq 90$~minutes (the raw data-collection floor), producing the full per-player tournament table.
The analysis pool is then narrowed to the \textbf{272~outfield players} who clear a higher \textbf{135-minute floor} ($\approx 1.5$~matches), with goalkeepers excluded.
...
Where it still admits a thin sample (Pedri's tournament-leading off-ball score rests on 185~minutes), the framework's answer is not to raise the floor but to \textbf{always display the raw values and the sample size next to every headline}, so a striking percentile on thin minutes can be discounted rather than trusted blindly.

\subsection{The Off-Ball Identity Problem (Preview)}\label{sec:identity-preview}

Because a 360~frame names only the ball-carrier, crediting off-ball work requires inferring \emph{who} each anonymous dot is.
...
It is validated against the only place where ground truth exists (completed passes, which \emph{do} carry a recipient name), reaching \textbf{93\% accuracy on the confident assignments it keeps}, far above the ${\sim}74\%$ of a naive last-known-position baseline.
...
\begin{itemize}
    ...
    \item \textbf{Reading the leaderboards.} Every table is read \emph{within role}, with the sample size shown next to the headline. A high percentile on a small sample (low minutes, or few decision events) is flagged throughout, so a striking headline can be discounted when the volume behind it is thin.
\end{itemize}
...
\end{equation}
It is a \textbf{calibrated gradient-boosted model} trained on \textbf{30,913 real open-play passes} (27,901~completed, 3,012~failed), reaching \textbf{AUC $\approx 0.92$} under match-grouped cross-validation.

\subsubsection{The 12 Features}\label{sec:xpass-features}

Every feature is computed at the \textbf{moment the sender touches the ball} (the freeze frame) and never from the pass's outcome, establishing a deliberate anti-leakage rule.
...
\begin{tabularx}{\textwidth}{@{}llLl@{}}
\toprule
\textbf{Group} & \textbf{Feature} & \textbf{What it captures} & \textbf{Expected sign} \\
\midrule
...
Four candidate features were computed, tested, and deliberately left out:
\begin{itemize}
    \item \texttt{defenders\_in\_corridor\_5m} and \texttt{defenders\_in\_corridor\_2m}: hard-threshold counts, which are \textbf{redundant} with the smooth \texttt{min\_perp\_dist\_to\_line} and \texttt{pass\_lane\_density\_score}.
    \item \texttt{receiver\_pressure\_count\_25}: logistic coefficient of $0.005$ (pure noise), which is redundant with \texttt{min\_dist\_def\_to\_receiver} in 95\% of the corpus.
    \item \texttt{pass\_height\_ground}: dropped to remove perfect collinearity (three height indicators summing to~1).
\end{itemize}
...
\begin{itemize}
    \item \textbf{Logistic regression} (L2-regularised): the interpretable baseline.
    \item \textbf{Balanced logistic}: class weights for the minority class (failed passes, only ${\sim}10\%$).
    \item \textbf{Gradient-boosted trees} (\texttt{HistGradientBoosting}): captures non-linearities and interactions.
\end{itemize}

All three were scored with the \textbf{same 5-fold cross-validation, grouped by match} (\texttt{GroupKFold} on \texttt{match\_id}).
...
The \textbf{GBM won} on AUC, log-loss and Brier score, reaching \textbf{AUC $\approx 0.92$}.
...
We therefore computed SHAP (SHapley Additive exPlanations) values for every sample, giving a per-feature contribution to the model output (positive SHAP pushes toward ``complete'', negative toward ``failed'').
The global feature-importance ranking, visualised in \cref{fig:xpass-shap}, confirms the expected signs from \cref{tab:features}: \texttt{pass\_distance} and \texttt{min\_dist\_def\_to\_receiver} are the two strongest drivers, followed by \texttt{pass\_lane\_density\_score} and \texttt{sender\_pressure\_count\_25}.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{images/xpass_shap.png}
...
The raw model output is passed through a \textbf{sigmoid calibrator} and then \textbf{clipped to $[0.05,\,0.99]$}:
\begin{itemize}
    \item \textbf{Sigmoid calibrator} (chosen over isotonic and beta). The corpus is heavily imbalanced (${\sim}90\%$ completions), so raw probabilities pile up near~1. In that saturated regime, isotonic (a flexible step function) tends to over-fit the extremes, whereas the two-parameter sigmoid fits a smooth, stable monotone correction.
...
\end{equation}
where the second term is the \textbf{mirrored failure cost}: the ball location $(105 - x, y)$ seen from the opponent's perspective if possession is turned over there.
The two terms are weighted symmetrically, \textbf{1\,:\,1} (turnover-penalty scale $= 1.0$): a turnover is treated as costing exactly what the completion would have gained, at the mirrored point.

This shape stops the metric from loving reckless ambition: a 40-metre ``Hollywood ball'' into a dangerous zone has a high reward term but a low completion probability and a heavy mirrored penalty, so its xEPV is honestly mediocre.
The robustness analysis (\cref{sec:h2-validation}) confirms that the turnover scale sets the \emph{level} of the numbers but \textbf{not the ranking} of players, so it is not a tuned knob hiding a result.

The xEPV definition lives in \textbf{one place} in the code and is imported unchanged by both H2 and H3, so the value attached to a given candidate is identical across the two studies (median difference $0.000$ on shared events).
...
We measure influence on three layers: the \textbf{shape} of the opposing block (convex hulls of the visible defenders), the \textbf{threat} added with each ball progression (EPV), and the \textbf{pull} a player exerts on the defenders around him (gravity).
...
For each pass we take the visible \textbf{opponents} in the freeze frame and compute their \textbf{convex hull}: the smallest polygon that encloses them all.
This polygon is our working model of the \textbf{defensive block}.
...
The pitch is divided into a \textbf{$12 \times 8$ grid of zones} (about $8.75\,\text{m} \times 8.5\,\text{m}$ each), used to build positional baselines for the gravity computation (\cref{sec:gravity-block}).
...
Each open-play pass is assigned an EPV value at its start and end location, read from a fixed EPV grid (a $32 \times 50$ lookup from Friends-of-Tracking-Data, values ranging from ${\sim}0.004$ in the own goal area to ${\sim}0.57$ near the opponent goal) by \textbf{bilinear interpolation} (so value varies smoothly across the pitch rather than jumping between grid cells).
The \textbf{EPV added} by a pass is the increase in scoring probability from its origin to its destination:
...
Beyond the scalar \texttt{epv\_added}, every pass is also tagged with a \textbf{geometric type} based on its topology relative to the hull:
\begin{itemize}
    \item \textbf{Penetration}: pass starts outside the hull and ends inside it.
    \item \textbf{Exit}: starts inside and ends outside.
    \item \textbf{Inside Circulation}: starts and ends inside.
    \item \textbf{Outside Circulation}: starts and ends outside.
\end{itemize}
...
The obvious question is \emph{which} passes count, and we deliberately refused to settle it with a single definition, because ``breaking a line'' can mean two genuinely different things: a \textbf{bold geometry} (splitting bodies) or a \textbf{high value} (creating danger). The best passes do both.
So we compute three flags:

\begin{enumerate}
    \item \textbf{Geometric line-breaker}: a successful pass that bypasses \textbf{at least 3~opponents inside a 5-metre corridor} along the pass line. The corridor width of $5$\,m was chosen because it captures defenders who are genuinely in the path of the ball (close enough to attempt an interception), while excluding those positioned laterally. The threshold of 3~opponents ensures that only passes which meaningfully disrupt the block's shape are counted; fewer than~3 would admit routine horizontal passes through a single line. This is the \emph{bold-geometry} lens: it counts passes that physically go through people, regardless of where on the pitch.
    \item \textbf{Quality (EPV) line-breaker}: a successful pass whose EPV added is \textbf{above the role-specific 75th~percentile} of positive EPV-added passes. The 75th~percentile is taken within role so that a centre-back is judged against centre-backs, not against attackers who naturally add more EPV per pass. The choice of the 75th (rather than, e.g., the median or 90th) balances selectivity (only the top quarter of value-adding passes qualify) against volume (enough passes survive to form a stable per-90 rate). This is the \emph{high-value} lens.
    \item \textbf{Clean line-breaker}: the \textbf{intersection} of the two, which is a pass that is both geometrically bold \textbf{and} high value.
\end{enumerate}
...
The mean number of opponents removed per progressive pass, \textbf{Def.\,Bypassed Avg}, is tracked alongside as the per-pass ``boldness'' companion to the per-90 volumes.
...
We remove that confound with a \textbf{leave-one-out (LOO) baseline}.

For each player, the expected defender behaviour in a given (match, zone) cell of the $12\times8$ grid is computed from \textbf{everyone else's events in that same zone, excluding the player's own}.
...
The proximity component uses the \textbf{$K = 4$ nearest opponents} to measure local defensive compactness: for each event, the mean distance to the four closest visible defenders is compared against the LOO baseline.
The choice of $K = 4$ is a robustness compromise: $K = 1$ would be noisy, fluctuating whenever a single defender happens to pass nearby; $K = \text{all}$ would dilute the signal with defenders on the far side of the pitch who have nothing to do with local pressure.
$K = 4$ captures the core of the local defensive cell (roughly the number of opponents who genuinely compress the available space around a ball-carrier) while staying insensitive to outliers.
As a safety fallback, $\min(K, |\text{visible opponents}|)$ is used, so frames with fewer than four visible defenders still contribute rather than being dropped.

Two distinct gravity signals come out of this, and we keep them separate on purpose:
\begin{itemize}
    \item \textbf{Compactness gravity} (the proximity and hull components): \emph{how tightly} the defence packs around the player relative to the LOO baseline.
    \item \textbf{Directional gravity} (the signed centroid displacement, \texttt{Def.\,Pull\,$|$m$|$}): \emph{whether} the defensive centroid actually shifts \textbf{toward} the player, in metres, versus the same LOO baseline.
\end{itemize}

The baseline is taken at \textbf{match level when there are enough frames} (a minimum of \textbf{10~frames} per team-zone cell, set by \texttt{MIN\_BASELINE\_FRAMES\,=\,10}) and falls back to a \textbf{tournament-level baseline} when a single match has too few.
...
The single most important design decision in H1 is that \textbf{three of them are treated as composites and one as a magnitude}. This is not a cosmetic label; it changes how the headline number is built:

\begin{itemize}
    \item \textbf{Progression}, \textbf{Reception}, \textbf{Gravity} are \textbf{stylistic composites}. Each models a latent style that shows up across several correlated indicators (a ``reflective'' model in measurement terms). For these, \emph{averaging the within-role percentiles} of the mother variables is the correct aggregation: no single variable \emph{is} the construct, they jointly indicate it.
    \item \textbf{Dangerousness} is a \textbf{magnitude}. The total EPV a player creates is a real, directly measurable quantity, and it is \textbf{by construction the sum} of four EPV sub-components. Averaging percentiles here would be wrong, as it would throw away the fact that the parts literally add up to the whole. So the Dangerousness headline is the \textbf{within-role percentile of EPV Added\,/\,90 itself}, which is a single honest magnitude.
\end{itemize}
...
These are \textbf{structural} missing values; they arise because the player's role naturally excludes certain geometric situations, not because of data error.
...
One structural regularity serves as a pipeline sanity check: the fourth EPV sub-component, \texttt{epv\_outside\_circ}, has \textbf{zero NaN\,s} across the 272-player pool, because every outfield player in open play makes at least one pass that both starts and ends outside the opponent hull. Indeed, outside-block circulation is the universal baseline of possession football.
...
Five mother variables, averaged as within-role percentiles:

\begin{enumerate}
    \item \textbf{LB Geom\,/\,90}: geometric line-breakers per 90 (the bold-geometry lens: $\geq 3$ opponents in a $5$\,m corridor).
    \item \textbf{LB Quality\,/\,90}: quality/EPV line-breakers per 90 (EPV added above role-specific 75th percentile of positive-EPV passes).
    \item \textbf{LB EPV\,/\,90}: clean line-breakers per 90 (the intersection: bold \emph{and} high value).
    \item \textbf{Hull Penetr.\,/\,90}: per-90 volume of successful passes that \emph{enter} the opposing convex hull.
    \item \textbf{Def.\,Bypassed Avg}: mean number of opponents removed from play per progressive pass.
\end{enumerate}
...
The headline is the \textbf{within-role percentile of EPV Added\,/\,90} alone, which is a direct magnitude rather than a composite.
The radar that decomposes it shows the \textbf{four additive EPV-by-zone components} (they sum to EPV Added\,/\,90 by construction):

\begin{enumerate}
    \item \textbf{EPV Penetr.\,/\,90}: value added on passes that \emph{enter} the opposing block.
    \item \textbf{EPV In-Circ\,/\,90}: value circulated \emph{inside} the block.
    \item \textbf{EPV Exit\,/\,90}: value of clean ball-exits from inside the block back out.
    \item \textbf{EPV Out-Circ\,/\,90}: value circulated outside the block.
\end{enumerate}

Reading the four together explains \emph{where} a player's EPV comes from, and this is the scouting payoff of decomposing rather than just totalling: a centre-back produces value mostly through Out-Circ and Penetr.; a between-the-lines attacking midfielder mostly through In-Circ and Exit; a winger drifting inside through Penetr.\ and In-Circ.
...
\begin{enumerate}
    \item \textbf{Between Lines\,\%}: share of touches taken inside the opponent convex hull.
    \item \textbf{Hull Exits\,/\,90}: per-90 volume of clean ball-exits from inside the block.
    \item \textbf{Press.\,Resist\,\%}: completion rate when at least 2~opponents are within 2.5\,m at the moment of release (the project-wide ``under pressure'' definition from \cref{sec:conventions}).
\end{enumerate}
...
\begin{enumerate}
    \item \textbf{Space Attraction\,\%}: how often defenders sit in the player's neighbourhood relative to the LOO baseline.
    \item \textbf{Gravity Hull\,\%}: the hull-based version of the same proximity idea.
    \item \textbf{Def.\,Pull\,$|$m$|$}: the absolute displacement, in metres, of the opposing defensive centroid toward the player.
\end{enumerate}
...
A forward who drags a centre-back out wide and a winger who pins both full-backs inside are both ``gravitational,'' but they are doing \textbf{different} things, so their mother variables do not move together.
...
\toprule
\textbf{Index} & \textbf{Mean $\alpha$} & \textbf{Interpretation} \\
\midrule
Progression & \textbf{0.77} & Tight construct, where the 5 variables genuinely measure one dimension. \\ \midrule
Dangerousness & \textbf{0.32} & Diagnostic only; the four radar axes are additive EPV components, not a reflective composite. Low $\alpha$ is expected, not a problem. \\ \midrule
Reception & \textbf{0.41} & Moderate composite, scoring high on CB and CAM, lower on FW due to small sample. \\ \midrule
...
The four indices are also only \textbf{weakly correlated with each other} (absolute correlation at most $0.56$ across the full pool, against a target below $0.6$), meaning there is no redundancy. Each index carries its own signal, and the four-card profile does not present four views of the same number.
...
This is the experiment that tests H1's core claim, which is that \textbf{ball volume is a poor proxy for value created}.
...
\toprule
\textbf{Index} & \textbf{Naive proxy} & \textbf{Spearman $\rho$} & \textbf{Mean $|\Delta|$} & \textbf{\% $|\Delta| > 20$} \\
\midrule
Progression & passes\,/\,90 & 0.47 & 21.7 & 47\% \\ \midrule
Dangerousness & passes\,/\,90 & \textbf{0.28} & \textbf{27.8} & \textbf{54\%} \\ \midrule
...
\textbf{Dangerousness} is where the gap hits hardest: \textbf{more than one player in two shifts by more than 20~percentile points} moving from the naive ranking (ball volume) to the contextual one (EPV magnitude).
The correlation of just $0.28$ means volume and value barely agree on who is dangerous.

\paragraph{Honesty on proxy choice.}
Progression and Dangerousness are tested against \emph{pure external volume} (passes\,/\,90), a stat that is \textbf{not part of the index}. This makes the test a clean ``volume versus context'' comparison.
We deliberately did \textbf{not} use EPV\,/\,90 as the Dangerousness proxy, because the index headline \emph{is} the percentile of EPV\,/\,90, and comparing it to itself would be tautological.
...
This is where the contextual approach moves the leaderboard most: the naive-to-contextual disagreement exceeds 20~points on \textbf{54\%} of the pool, the largest of all four indices.
...
Tom\'{a}\v{s} Sou\v{c}ek (Czech Republic, MID, $\textbf{+80}$ shift), Heorhii Tsitaishvili (Ukraine, FB, $\textbf{+75}$) and Ivan Peri\v{s}i\'{c} (Croatia, FB, $\textbf{+74}$) are the largest naive-to-contextual shifts in the pool.
Sou\v{c}ek is the headline: he is in the 3rd percentile by passes\,/\,90 among MIDs but in the \textbf{83rd percentile by EPV\,/\,90}. He has almost no passing volume, yet every involvement moves the value needle (his radar peaks on Exit at 100 and In-Circ at 72, which is typical of a box-arriving holding midfielder who breaks out of pressed phases). This single player is the clearest proof of the whole H1 thesis.
...
Jan Mlakar (Slovenia, WIDE) sits at the 95th percentile. He is the index leader and ranks at the 100th percentile of WIDE players on the proximity component.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{images/linebreaking_pass_1.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{images/scouting_discoveries.png}
...
For every open-play pass we reconstruct the \textbf{alternative set}: every in-frame teammate the player \emph{could} have passed to.
Each teammate is scored with the shared machinery (xPass, then xEPV) \textbf{as if the sender had attempted that pass}.
...
\begin{itemize}
    \item \textbf{Headed passes are excluded}: passes played with the head (\texttt{body\_part\,=\,'Head'}) are removed because headers have a fundamentally different geometry and decision logic from foot passes. The sender's control over direction, height and weight is far more constrained, so grading the ``choice'' on the same feature set would be misleading.
    \item \textbf{Pass-into-space is excluded}: events where no in-frame teammate is within \textbf{12\,m} of the chosen pass's end location. The 12\,m threshold is not arbitrary: it is about the furthest a receiver can travel during ${\sim}1.5$\,s of ball flight at sprint speed (${\sim}8$\,m/s), so it keeps genuine medium-long passes to a moving receiver while dropping true into-space balls that have no well-defined ``who did he choose.''
    \item \textbf{Goalkeeper senders are excluded}: a keeper's distribution (long goal kicks, unpressured box passing) has a completely different geometry from an outfield decision.
\end{itemize}
...
We resolved this with a single empirically-grounded rule: candidates \textbf{under 30\,m} are scored as ground passes, candidates \textbf{over 30\,m} as lofted. This is because in the real corpus passes flip from ${\sim}77$--$93\%$ ground below 30\,m to predominantly high above it ($9\%$ ground / $91\%$ high beyond 40\,m).
One distance threshold captures that regime change cleanly without per-distance tuning.

This step produces the \textbf{alternatives table}: \textbf{222,274 candidate rows} across the corpus, about \textbf{7.2 options per event}.

\subsection{The DQ Index}\label{sec:dq-index}

The headline is the \textbf{DQ index}, a \textbf{relative choice-optimality index within a constrained action set}.
It is built in two steps:

\begin{enumerate}
    \item \textbf{Per event.} The \textbf{Score} is the share of the in-frame alternatives whose xEPV is at or below the xEPV of the pass the player actually chose (restricted to events with at least 2 alternatives):
    ...
    \item \textbf{DQ index} = the \textbf{within-macro-role percentile} of that mean Score.
\end{enumerate}
...
\begin{itemize}
    ...
    \item \emph{Not regret-versus-best.} A ``regret'' score (how far each chosen pass falls below the single best option available) was tested and \textbf{rejected}, because it punishes players who consistently operate in frames where \emph{many} options are valuable.
...
\textbf{Companion metric: Value Impact} $= \text{mean}(\text{xEPV}_{\text{chosen}} - \overline{\text{xEPV}}_{\text{alternatives}})$, the value-weighted view of the same signal.
As plotted in \cref{fig:dq-vs-value-impact}, Value Impact correlates with the DQ index at \textbf{Spearman $\rho \approx 0.76$} within role, confirming they capture a highly aligned tactical signal while retaining role-specific characteristics. Value Impact acts as the value-weighted companion that plays the role \texttt{epv\_added\,/\,90} plays next to the Dangerousness percentile in H1.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{images/dq_vs_value_impact.png}
...
\begin{itemize}
    \item \textbf{Picks the best}: rate of in-frame value-optimal picks.
    \item \textbf{Avoids the worst}: rate of in-frame value-bottom picks, mirrored ($100 - \text{percentile}$)..
    \item \textbf{Elite reads\,/\,90}: volume of top-decile decisions per 90.
    \item \textbf{Avoids poor\,/\,90}: volume of bottom-decile decisions per 90, mirrored.
\end{itemize}
These four split the single Score into the two things a scout actually wants separated: \textbf{how good} the picks are (accuracy and avoid-worst rates) and \textbf{how often} they happen (elite\,/\,90 and avoid-poor\,/\,90 volumes).
A \textbf{core-stats table} under the radar exposes the raw scout-facing values (Score, Value Impact, the four radar sources in raw units, average miss cost, and the number of decisions with minutes), so low-sample profiles can be discounted. (The ``average miss cost'' is the mean xEPV left on the table \emph{on the events that were sub-optimal}, representing the severity of a mistake, separate from its frequency.)

\begin{figure}[H]
    \centering
    \includegraphics[width=0.55\textwidth]{images/dq_radar_example.png}
...
Kimmich, Modric, Xhaka and Barella all play deliberate tempo control, and xEPV is a per-pass quantity with \textbf{no game state} (no scoreline, no clock, no tactical instruction), so a deliberately safe, possession-securing pass reads as a weak selection.
...
We measure the \textbf{dangerous off-ball space a player occupies that his teammates fail to use}: high-value runs that are made, seen by the freeze frame, and left unserved.
...
The headline rating is its \textbf{within-macro-role percentile}.

\paragraph{Why ``uncapitalised.''}
The whole point of H3 is to find value the event log misses.
Value that \emph{was} served already shows up downstream (someone received it and did something with it).
The interesting, invisible quantity is the value that was offered and \textbf{wasted}, which is the run nobody played.
...
A player with \textbf{high Potential and high Latency} is a \textbf{shadow runner}: he keeps offering dangerous options his team does not use.
A player with \textbf{high Potential and low Latency} is a \textbf{trusted threat}: he offers as much, and his team uses it.
...
URS carries a structural interpretation constraint that we state plainly: it is \textbf{conditional on teammate execution}.
A candidate counts toward URS only when the run is \emph{not} served. Whether a run is served depends partly on the ball-carrier (their vision and execution) and not on the off-ball player alone.
Crucially, the constraint does \textbf{not} spread evenly across the metric:
\begin{itemize}
    \item \textbf{Off-Ball Potential} is the \textbf{player-only} half: it depends on where he moves and the danger of the space he occupies, not on whether anyone serves him.
...
H2 grades a player as the \textbf{server}, measuring how well he chooses among the options he has on the ball.
...
A 360~frame gives positions but \textbf{no identity}: only the player on the ball is named; everyone else carries just a team tag.

\paragraph{The three-step resolver.}
\begin{enumerate}
    \item \textbf{Estimate each teammate's position} at the instant of the pass by \textbf{bracket interpolation}, interpolating between his nearest on-ball event \emph{before} and his nearest \emph{after} the pass, within the staleness window.
    This ``v2'' bracket method replaced a ``v1'' baseline that used only the player's \textbf{last-known position before the event} (backward-only extrapolation); bracketing is substantially more accurate because it uses the player's trajectory on \textbf{both} sides of the target instant, effectively treating the two nearest events as a linear motion prior rather than a static snapshot.
    \item \textbf{Match estimates to dots} with the \textbf{Hungarian algorithm}, which is the optimal one-to-one assignment that minimises total distance between estimated positions and observed dots. (A greedy nearest-dot assignment can paint itself into a corner; the Hungarian algorithm finds the globally best matching.)
    \item \textbf{Keep only confident assignments} (those localised within \textbf{8\,m}) and score only those.
\end{enumerate}

\paragraph{Validation.}
There is no ground truth for off-ball runs, but there \emph{is} ground truth for \textbf{completed passes}: the recipient is named.
...
On the open-play universe the metric runs on, the bracket resolver reaches \textbf{93.2\% accuracy on the confident assignments it keeps}, far above the ${\sim}74\%$ of the naive last-known-position baseline.
...
\begin{itemize}
    \item \textbf{Staleness window: 5~seconds.} A teammate's position is only estimated from an event within this many seconds of the pass. Sweeping the window from 20\,s down to 5\,s, accuracy on confident dots rises from \textbf{88\% to 96\%} while the volume of confident candidates stays essentially flat (${\sim}1.0\times$). So tightening the window buys fresher, more accurate estimates \textbf{at no cost in coverage}, representing an unusually clean trade. The final URS\,/\,90 player ranking is stable across the whole window range (Spearman $0.96$ versus the 20\,s setting).
    \item \textbf{Confidence threshold: 8\,m.} At 8\,m the mean localisation error of confident dots is about 4\,m and accuracy ${\sim}96\%$. Tightening to 6\,m does \textbf{not} raise accuracy but throws away ${\sim}17\%$ of candidates, showing that 8\,m is the point that keeps the most data without sacrificing accuracy.
\end{itemize}

A practical note on reproducibility: because StatsBomb regenerated every event UUID, H3 cannot merge onto H2's frozen alternatives table (it would return zero rows). Instead it \textbf{recomputes xPass on the fly} from the live freeze frames using H2's saved model (the same model with the same 13 features), so the value attached to each candidate is exactly what H2 would attach (median difference $0.000$ on shared events).

\subsection{Validation}\label{sec:h3-validation}

\paragraph{Split-half reliability.}
Splitting the 51~matches by date into the first 25 and last 26, the URS\,/\,90 ranking reproduces at \textbf{Spearman $\rho = 0.72$} ($n=117$ players who clear the floor in both halves), as visualised in the scatter plot of \cref{fig:urs-split-half}.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{images/urs_split_half.png}
...
Pedri (Spain, CAM, $n=185'$) tops the whole pool at \textbf{URS\,/\,90 $= 2.78$}, roughly twice the next name.
He exposes more off-ball value than anyone (Potential\,/\,90 $= 3.30$) and is served on only \textbf{16\%} of it: the extreme shadow runner of the tournament.
...
\begin{enumerate}
    \item Midfielders \textbf{appear as candidate teammates in more freeze frames}, since they are simply seen more often by the resolver.
    \item They \textbf{operate in higher-EPV zones} than centre-backs, so their hypothetical xEPV is structurally larger.
    \item They are the \textbf{easiest targets for the Hungarian assignment}: most events happen near them, so position estimates are fresher and the confident flag is set more often.
\end{enumerate}
All three effects compound and concentrate URS\,/\,90 in the MID role.  \textbf{The reading position by position is therefore the honest one}, comparing a forward against a forward, a centre-back against a centre-back.
...
\begin{itemize}
    \item \textbf{CB}: Tah, R\"{u}diger, Calafiori, R\'{u}ben Dias, Bastoni: the ball-playing centre-backs who step out and offer a forward option.
    \item \textbf{FB}: Kimmich, Dalot, Mittelst\"{a}dt, Di Lorenzo, Trippier: the attacking full-backs.
    \item \textbf{MID}: Jorginho, Hjulmand, Modri\'{c}, Kroos, Andrich: the creative engine room.
    \item \textbf{CAM}: Pedri, G\"{u}ndoğan, Eriksen: the between-the-lines tens.
    \item \textbf{WIDE}: Wirtz, Concei\c{c}\~{a}o, Foden, Su\v{c}i\'{c}, Musiala: the inside-drifting forwards.
    \item \textbf{FW}: Gregoritsch, Havertz, G\"{u}ler, Thuram, Oyarzabal.
\end{itemize}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{images/within_role_top5.png}
...
The cleanest illustration is \textbf{Pedri against Kroos}.
Both are near the top on Off-Ball Potential, so on the volume axis they look alike.
The difference is Latency: Pedri is served on 16\% of the value he offers, \textbf{Kroos on 50\%}.
Same exposure, opposite outcome: one a threat his team keeps missing, the other a threat his team uses.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{images/h2h_pedri_kroos.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.55\textwidth]{images/offball_radar_pedri.png}
...
\textbf{Hypothesis~4.}
With the three contextual metrics now in place, a player is already represented as a rich, team-bias-corrected feature vector, which is exactly the input a clustering and similarity model needs.
H4 groups players by their intrinsic ``spatial DNA'' and behavioural playing style, rather than by nominal positional label, and powers a \textbf{similarity model} that surfaces cost-effective alternatives with comparable tactical intelligence to premium assets.
This is the direct realisation of the value-arbitrage business case stated in \cref{sec:business}.

\subsection{Feature Vector Construction}\label{sec:h4-features}

Each of the 272~players in the shared pool is described by the full set of contextual metrics produced by H1, H2 and H3: the within-role percentiles on all mother variables across the four H1 indices (Progression, Dangerousness, Reception, Gravity), the H2 DQ~index and its four radar axes, and the H3 URS\,/\,90 with its three radar axes.
Because all metrics are already expressed as within-role percentiles, the feature vector is naturally normalised and comparable across roles: each dimension sits on a $[0, 100]$ scale with the role-specific mean removed by construction.
This multi-dimensional representation captures a player's ``spatial DNA'', representing his full contextual profile stripped of team-bias inflation.

\subsection{Clustering Methodology}\label{sec:h4-clustering}

Rather than relying on the six nominal macro-roles, we apply clustering algorithms to the contextual feature space to discover \textbf{emergent player archetypes}, which are groups that reflect how players actually behave on the pitch rather than how they are labelled on the team sheet.
Players who share similar contextual profiles (e.g.\ a ball-playing centre-back who steps into the half-space and a deep-lying midfielder who drops into the back line) may cluster together despite occupying different nominal positions, which is exactly the kind of insight a scout needs for recruitment.

% TODO: Insert specific clustering results, number of clusters, and archetype descriptions
%       when the H4 analysis is finalised.

\subsection{Similarity Model}\label{sec:h4-similarity}

The similarity engine computes distances in the contextual feature space to answer the scouting query: \emph{given a target player, who are the most similar alternatives?}
Combined with the Transfermarkt market values from \cref{sec:transfermarkt}, this directly operationalises the value-arbitrage use case: a scout can identify a low-cost player whose contextual profile closely matches a premium asset, with the comparison grounded in EPV-based, team-bias-corrected metrics rather than in raw volume statistics.

% TODO: Insert similarity results, example queries (e.g., "find a cheaper alternative to X"),
%       and validation of the similarity model when H4 is finalised.
...
With the similarity engine in place, the comparison page evolves from a manual head-to-head (where the scout picks two players) to an automated ``find me similar undervalued players'' query, powered by the clustering and distance scoring described above.
...
It is a contextual scouting tool: a scout can read a player's full contextual profile and run within-role head-to-heads.

\subsection{What a Scout Can Do}\label{sec:scout}

\begin{itemize}
    \item \textbf{Read a player's profile}: the H1, H2 and H3 cards side by side on one page. Every card follows the same pattern: a \textbf{magnitude headline} next to a \textbf{within-role percentile radar} that decomposes the profile behind it, with the \textbf{raw scout-facing values} shown underneath so low-sample profiles can be discounted.
    \item \textbf{Run head-to-heads}: two players of the same macro-role overlaid on the same radars, so the comparison always stays \emph{within role}. The picker prevents cross-role duels upstream, which keeps the comparison meaningful (a CB radar and a FW radar are not the same axes).
    \item \textbf{Set performance against cost}: the Transfermarkt market values let a scout read on-pitch contextual output against price, which is the value-arbitrage angle of the business case.
\end{itemize}
...
\begin{tabularx}{\textwidth}{@{}lLLL@{}}
\toprule
\textbf{Metric} & \textbf{Headline} & \textbf{Radar} & \textbf{Axes} \\
\midrule
...
\begin{itemize}
    \item \textbf{Frontend}: React + TypeScript, including the player list, filters, search, the three metric sections, and a player-comparison page.
    \item \textbf{Backend}: FastAPI (Python), which exposes the per-player metric endpoints.
    \item \textbf{Database}: Supabase / Postgres, loaded by a set of import scripts from the committed metric CSVs.
\end{itemize}
...
Each hypothesis lives in its own self-contained folder with its own dependencies, a thin orchestrating notebook that imports all logic from a \texttt{src/} package, and committed output CSVs that allow an \textbf{analysis-only path}. A reader can open a notebook and jump straight to the index, validation and figure cells without re-running the heavy pipelines (the hull computation, the xPass training, or the resolver pull).
...
The full repository, with all four studies and the dashboard, is the primary deliverable alongside this paper.
...
Several limits are \emph{declared by design}: they are acknowledged inside the metric construction rather than discovered after the fact, which is a deliberate strength of the framework.

\begin{enumerate}
    \item \textbf{No game state in xEPV.} xEPV is a per-pass quantity with no notion of scoreline, time remaining, or tactical intent. This is the source of the most counter-intuitive H2 reads: deliberate tempo controllers (Kimmich, Modric, Xhaka, Barella) sit below their role median because deliberately safe passing looks like weak selection to a per-pass value metric. The gap is the lens, not necessarily the player.

    \item \textbf{The decision set is a model construct (H2).} The DQ~index grades a pass against the \emph{in-frame} alternatives the resolver reconstructs, which implicitly assumes that every reachable in-frame teammate belongs to the player's real decision set. Some in-frame options are technically available but \textbf{tactically irrational} (a square ball to the far-side full-back while breaking on the counter) yet they still enter the option menu and inflate the Score. The framework already absorbs the converse case: every option is weighted by its own completion probability (xPass sits inside xEPV), and balls with no reachable receiver are dropped (the 12\,m pass-into-space filter), so unrealistic options are down-weighted to near-zero value rather than rewarded. What remains unmodelled is \emph{tactical rationality}. For this reason the index is read as a \textbf{relative choice-optimality measure within the model's reachable action set}.

    \item \textbf{URS is conditional on teammate execution (H3).} A run counts toward URS only when it is not served, and whether a run is served depends partly on the ball-carrier. The dependency lives almost entirely in the \textbf{Latency} axis, while Off-Ball Potential is a player-only measure ($\rho \approx 0.08$ within role). The environment effect is real but small ($\rho \approx -0.13$ between a player's Latency and his teammates' mean H2). URS is a natural complement to H2: a high-Potential player in a low-H2 team reads as off-ball value being under-used by his current side.

    \item \textbf{Sample size at the floor.} The 135-minute floor admits players on very thin samples. The framework's answer is to always show the raw values and sample size next to every headline, establishing a presentation safeguard rather than a fix for the underlying variance.

    \item \textbf{Resolver confidence trade-off (H3).} The receiver resolver only scores candidates it can localise within 8\,m, reaching 93\% accuracy on those confident assignments. This buys reliability at the cost of coverage: dots it cannot place confidently are dropped.

    \item \textbf{EPV grid granularity.} Both the H1 value layer and the H2/H3 xEPV layer read value from a fixed EPV grid; the framework inherits whatever spatial resolution and assumptions that grid encodes.

    \item \textbf{Single tournament.} All results are on Euro~2024 (51~matches). The metrics are validated for internal stability (split-half, robustness), but cross-tournament and cross-season generalisation is not yet tested.

    \item \textbf{Within-role pools are uneven.} The six macro-role pools differ in size, so a percentile in a small pool (e.g.\ CAM) is a coarser rank than one in a large pool. This is inherent to within-role benchmarking and is why sample size is always shown.
    
    \item \textbf{Minority class.} The 10\% of events that are pass-into-space are handled by a dedicated filter, as these have no clear recipient dot in the 360 data.
\end{enumerate}
...
\begin{itemize}
    \item \textbf{Game state in xEPV}: conditioning the value layer on scoreline and game phase, which would close the main H2 scope limit (the tempo-controller blind spot).
    \item \textbf{Cross-tournament validation}: re-running the four metrics on other StatsBomb 360 competitions to test generalisation beyond Euro~2024.
    \item \textbf{Defensive off-ball value}: H3 currently measures \emph{offensive} off-ball exposure; the mirror concept (the dangerous space a player \emph{denies} the opponent) is a natural extension.
    \item \textbf{Combined player-plus-fit score}: quantifying the H2 $\times$ H3 pairing into a single score that captures both the player's decision quality and how well his off-ball value is used by his current team.
\end{itemize}
...
This project reframes player evaluation around \textbf{what a player does with the space around him}, graded by \textbf{value} (EPV) rather than volume, and benchmarked \textbf{within role} rather than against the whole pitch.

Four completed and independently validated studies implement this thesis:
\begin{itemize}
    \item \textbf{Space Control} (four spatial-influence indices),
    ...
    \item \textbf{Player Similarity} (grouping players by contextual ``spatial DNA'' for value arbitrage).
\end{itemize}
...
The framework is built to be honest about its own limits, and it sets up clear next steps including game-state-aware value models, cross-tournament validation, and AI-generated scouting reports.
...
\bibitem{epv}
Fern\'{a}ndez, J., Bornn, L., and Cervone, D.
\emph{Decomposing the Immeasurable Sport: A deep learning expected possession value framework for soccer}.
...
Advances in Large Margin Classifiers, pp.~61--74, MIT Press, 1999.

\bibitem{cronbach}
Cronbach, L.\,J.
\emph{Coefficient alpha and the internal structure of tests}.
Psychometrika, 16(3):297--334, 1951.

\bibitem{spearman}
Spearman, C.
\emph{The Proof and Measurement of Association between Two Things}.
The American Journal of Psychology, 15(1):72--101, 1904.

\bibitem{repo}
Vezzoli, M.\ and Mio, A.
\emph{Contextual Football Scouting: Full codebase}.
\url{https://github.com/ArMat-Analytics/Contextual-Football-Scouting}, 2026.

\bibitem{proposal}
Vezzoli, M.\ and Mio, A.
\emph{Project Proposal: Contextual Football Scouting: Mitigating Team Bias via 360 Spatial Data Analysis}.
April 2026.
...
\begin{tabularx}{\textwidth}{@{}lLll@{}}
\toprule
\textbf{Figure} & \textbf{File} & \textbf{Origin} & \textbf{Section} \\
\midrule
...
\cref{fig:archetype-map} & \texttt{archetype\_map.png} & H3 archetypes & \cref{sec:h3-findings} \\
\bottomrule
\end{tabularx}
\end{table}
