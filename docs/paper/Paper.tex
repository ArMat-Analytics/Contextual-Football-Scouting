...
% Table of contents goes down to subsubsection only; \paragraph titles
% stay out of the ToC (they are in-text headings, not navigable sections).
\setcounter{tocdepth}{3}
% Belt-and-braces: make the \paragraph ToC line a no-op so paragraph
% headings never appear in the table of contents regardless of tocdepth.
\makeatletter
...
On this basis we present four validated studies on the same pool of 272 players: (H1)~\textbf{Space Control~\&~Value}, a four-index spatial profile; (H2)~\textbf{Decision Quality}, the value of the pass chosen against the in-frame options ignored; (H3)~\textbf{Off-Ball Movement}, the dangerous off-ball space a player offers that teammates fail to use; and (H4)~\textbf{Player Similarity}, matching players by their contextual ``spatial DNA'' to surface cheaper look-alikes. Beyond confirming what football already knows, the metrics isolate a previously invisible signal, the ``shadow runner'' who keeps offering dangerous options his team never plays, with Pedri the clearest case of the tournament. The four studies feed an interactive scouting dashboard. We conclude that grading players by the value of what they do, and only against others in their role, removes much of the team-bias inflation in raw statistics and surfaces undervalued players that volume-based scouting overlooks.

\medskip
\noindent\textbf{Keywords:} football analytics; player scouting; Expected Possession Value; StatsBomb 360 spatial data; team bias.
\end{abstract}
...
\clearpage
% ============================================================
% FRONT MATTER: navigation aids (TFM guide §19)
% ============================================================
\tableofcontents
\clearpage
\listoffigures
...
% ------------------------------------------------------------
% List of Abbreviations and Acronyms (TFM guide §19)
% ------------------------------------------------------------
...
Acronym & Meaning \\
\midrule
\multicolumn{2}{@{}l}{\textit{Core metrics and models}} \\
\midrule
EPV   & Expected Possession Value: the value of having the ball at a given pitch location, the project's common currency (\cref{sec:epv}). \\
xEPV  & Expected Net Possession Value: the pass-probability-weighted EPV gain of an action, $xPass \times \Delta EPV$ (\cref{sec:xepv}). \\
xPass & Expected Pass: the model-estimated probability that a given pass is completed (\cref{sec:xpass}). \\
DQ    & Decision Quality: H2's relative choice-optimality index over a player's in-frame passing options (\cref{sec:h2}). \\
URS   & Unrealised Reception Score: H3's index of dangerous off-ball space a player offers but never receives (\cref{sec:urs}). \\
DNA   & The 11-axis ``spatial DNA'' vector used in H4 to match players by playing style (\cref{sec:h4-features}). \\
VAEP  & Valuing Actions by Estimating Probabilities: a prior possession-value model cited in related work (\cref{sec:related}). \\
xT    & Expected Threat: a zone-based possession-value model cited in related work (\cref{sec:related}). \\
\midrule
\multicolumn{2}{@{}l}{\textit{Macro-roles (the within-role lens)}} \\
\midrule
CB    & Centre-back. \\
FB    & Full-back. \\
MID   & Central midfielder (absorbs the CDM and CM fine roles). \\
CAM   & Attacking midfielder. \\
WIDE  & Wide attacker (absorbs the W and WM fine roles). \\
FW    & Forward. \\
\midrule
\multicolumn{2}{@{}l}{\textit{Method and data}} \\
\midrule
LB    & Line-break: a pass that bypasses the defensive line (H1 building block, \cref{sec:linebreakers}). Not a positional label. \\
LOO   & Leave-One-Out: the baseline used to measure a player's Gravity by removing him from the frame (\cref{sec:gravity-block}). \\
SHAP  & SHapley Additive exPlanations: the feature-attribution method used to read the xPass model (\cref{sec:model-selection}). \\
GBM   & Gradient Boosting Machine: the model chosen for xPass (\cref{sec:model-selection}). \\
AUC   & Area Under the ROC Curve: the ranking metric used in xPass model selection (\cref{sec:model-selection}). \\
EDA   & Exploratory Data Analysis. \\
StatsBomb 360 & The freeze-frame data source giving the positions of off-ball players (\cref{sec:statsbomb}). \\
\bottomrule
\end{tabularx}
\end{table}
...
Team bias has two parts. The first is \emph{how each action is valued}: a strong team lets a player make many easy, low-value passes, and raw counts reward them all the same. The second is \emph{how many chances a player gets}: the same team gives him the ball more often, so he has more chances to do something useful than an equally good player in a weaker side. We tackle the first part directly, by grading actions on their value instead of their count and comparing players only within their role. The second part, the extra chances a strong team gives, is harder to remove; we come back to it in \cref{sec:limitations}.
...
\subsection{State of the Art and Positioning}\label{sec:related}

Valuing player actions by space rather than volume is an established line in football analytics, and it helps to state where this project sits. Possession-value models such as Expected Threat~\cite{xt} and VAEP~\cite{vaep} lay a value surface over the pitch and credit a player for moving the ball into more dangerous zones; pitch-control models~\cite{pitchcontrol} estimate, from tracking data, the area a team would control if the ball arrived there; and Rein, Raabe and Memmert~\cite{packing} count how many defenders a pass outplays (the academic form of the ``packing'' idea). The most complete academic treatment of possession value is the Expected Possession Value framework of Fern\'{a}ndez, Bornn and Cervone~\cite{epvframework}, who decompose EPV into separately estimated subcomponents and learn pass-value surfaces with deep networks from full spatiotemporal tracking data. Our EPV layer is in the same conceptual family, but works from the static information in a single 360~freeze frame rather than continuous tracking, trading their temporal richness for openly available data; our geometric line-breaker is, in turn, a packing-style count.

Closest in spirit to our off-ball study (H3) is the space-valuation line: Fern\'{a}ndez and Bornn~\cite{wideopenspaces} quantify, from continuous tracking, the value of the space a player occupies for himself (Space Occupation Gain) and generates for teammates (Space Generation Gain), and explicitly leave the notion of \emph{potential space}, the valuable space a player could reach but does not yet occupy, as an open direction. Our Off-Ball Movement metric is in this family, but reads the space a player offers from the single 360~frame and scores specifically the dangerous space his teammates fail to use.

Two further lines are close to specific parts of our pipeline. Anzer and Bauer~\cite{xpasspaper} model the difficulty of a pass (its completion probability) from spatio-temporal data, the same quantity our xPass model estimates and uses inside xEPV. Dick, Link and Brefeld~\cite{availability} model player \emph{availability}, the probability that a pass to a given teammate would arrive without being intercepted, which is close both to how we reconstruct off-ball receivers (H3) and to how we build the in-frame option set a pass is judged against (H2). Where these works rely on continuous tracking or proprietary feeds, our versions run on the single freeze frame the open 360 data provides.

Three choices set this work apart. First, every value is read from StatsBomb 360 freeze frames rather than continuous proprietary tracking, so the method runs on openly available data. Second, we grade not only the action played but the action \emph{relative to the options ignored} (H2) and the off-ball value a player offers but never receives (H3); standard possession-value models score the on-ball event and have no notion of either the unchosen alternative or the unserved run. Third, rather than simply counting the defenders a pass outplays, we weight each line-break by EPV and separate the bold-geometry, high-value and clean cases, so a frequent but cheap line-breaker is told apart from one whose every break is dangerous.
...
From this thesis follows our \textbf{general objective}: to build, from StatsBomb~360 spatial data, a within-role contextual evaluation framework that grades Euro~2024 players by the value they create rather than the volume they accumulate, and to deliver it as a usable scouting tool. We break this into four \textbf{specific objectives}, one per study, each with a measurable, validated outcome:
\begin{itemize}
    \item \textbf{(H1)} Quantify a player's spatial influence (block broken, threat added, defensive pull) from each freeze frame, and verify the resulting indices rank players differently from volume-based statistics.
    \item \textbf{(H2)} Grade the value of the pass a player chooses against the in-frame alternatives he ignored, and confirm the resulting index tracks the value actually created within role.
    \item \textbf{(H3)} Measure the dangerous off-ball space a player offers but does not receive, which first requires resolving anonymous off-ball identities at validated accuracy, and test that the score is internally reliable.
    \item \textbf{(H4)} Match players within role by their contextual ``spatial DNA'' on a fixed $0$--$100$ scale, and check that the matches read as style rather than team or position.
\end{itemize}
Each specific objective is the measurable target that the corresponding study (\crefrange{sec:h1}{sec:h4}) sets out to meet; the headline metric and validation of each are summarised in \cref{tab:hypotheses}.
...
Each specific objective is pursued as a testable hypothesis; the four are summarised in \cref{tab:hypotheses}.
...
\begin{enumerate}
    \item A contextual, team-bias-adjusted scouting framework that grades players by EPV value, not event volume, and benchmarks every player within his macro-role rather than against the whole pitch.
...
Before building any metric, we describe the raw data: its scale, how we filter the analysis pool from it, how it distributes across roles, and where it is incomplete. We compute all figures below directly from the committed player tables and frame data. This section covers only the shared raw data; the contextual variables this project actually studies (EPV, xPass, the within-role indices) are built later, so we describe and explore each one where it is defined, in \crefrange{sec:h1}{sec:h4}, with its distribution, typical values and leaderboards shown alongside its construction.

\paragraph{Scale and on-ball volume.} Across the 51 matches, the event log holds \textbf{51{,}534} attempted passes by the 357 players who reach the 90-minute data-collection floor. Of these, \textbf{39{,}013} passes belong to the 272-player analysis pool and carry a usable 360~freeze frame; these are the passes every spatial metric reads. A share of on-ball events therefore has no spatial snapshot attached, and any metric that needs the frame uses only the passes that have one.

\paragraph{Pool construction.} We filter the analysis pool in two stages, summarised in \cref{tab:eda-construction}. The raw collection keeps every player with at least 90~minutes (357~players). We then raise the bar to a 135-minute floor ($\approx 1.5$ matches) and remove the 28~goalkeepers (excluded for the reason given above). What remains is the 272-player pool we share across all four studies.

\begin{table}[H]
\centering
\caption[Analysis-pool construction]{From the raw collection floor to the shared analysis pool.}\label{tab:eda-construction}
\small
\begin{tabularx}{\textwidth}{@{}Lr@{}}
\toprule
Stage & Players \\
\midrule
Raw collection floor ($\geq 90$~min, all positions) & 357 \\
Remove goalkeepers & $-28$ \\
Remove outfield players below the 135-minute analysis floor & $-57$ \\
\midrule
\textbf{Shared analysis pool} ($\geq 135$~min, outfield) & \textbf{272} \\
\bottomrule
\end{tabularx}
\end{table}

\paragraph{Role distribution.} We collapse StatsBomb's 23~fine positions into six macro-roles (\cref{sec:roles}); the resulting pool has more midfielders and defenders than attackers, and the six pools are uneven in size (\cref{tab:eda-roles}). The unevenness has a direct effect on the metrics: a within-role percentile in the 20-player CAM pool is a coarser rank than one in the 65-player MID pool, so we carry role-pool size as a declared limit (\cref{sec:limitations}).

\begin{table}[H]
\centering
\caption[Macro-role distribution of the pool]{Macro-role distribution of the 272-player analysis pool.}\label{tab:eda-roles}
\small
\begin{tabularx}{\textwidth}{@{}Lrr@{}}
\toprule
Macro-role & Players & Share \\
\midrule
MID (central midfielder)        & 65 & 23.9\% \\
CB (centre-back)                & 61 & 22.4\% \\
FB (full-back)                  & 57 & 21.0\% \\
WIDE (wide attacker)            & 37 & 13.6\% \\
FW (forward)                    & 32 & 11.8\% \\
CAM (attacking midfielder)      & 20 & \phantom{0}7.4\% \\
\midrule
\textbf{Total}                  & \textbf{272} & 100\% \\
\bottomrule
\end{tabularx}
\end{table}

\paragraph{Minutes played.} Within the pool, minutes played run from the 135-minute floor to 691, with a median of 282 ($\approx$ three matches). Players near the floor have a noisier ranking, and on a 272-player board a noisy entry can also shift the players ranked next to it. Rather than raise the floor (which would drop group-stage players relevant to the tournament), we display the raw values and the sample size next to every headline, so a high percentile on few minutes can be discounted.

\paragraph{Frame coverage and missing data.} Missingness here takes the form of \emph{partial frames} rather than blank cells: a 360~snapshot typically captures only the 8--12 players nearest the ball, so a fraction of each player's passes cannot be spatially analysed. Per player, the share of passes that carry a usable frame (\texttt{coverage\_pct}) has a median of \textbf{87.5\%} and ranges from \textbf{62.6\%} to \textbf{98.2\%}. Coverage is high but never total, and a player at the lower end ($\approx 63\%$) rests on a thinner spatial sample. The effect is larger for off-ball work, which is why the H3 resolver keeps only confident, ball-proximate identity assignments (\cref{sec:resolver}).

\paragraph{Raw pass completion.} Before any contextual layer, the raw pass-completion rate across the pool spans \textbf{51\% to 97\%} (median 83\%). Part of this spread reflects how dominant a player's side is rather than the player's own quality: an easy ball in a possession-heavy team is completed more often than a hard one in a pressed side. This is the team-bias problem the project targets, and grading by value and ranking within role (\cref{sec:common}) is the response to it.
...
This is the same quantity studied by Anzer and Bauer~\cite{xpasspaper}; their model draws on continuous tracking data, whereas ours reads only the static freeze frame at the moment the pass starts, since that is what the 360~data provides.
...
\begin{table}[H]
\centering
\caption[xPass model selection]{xPass model selection (5-fold \texttt{GroupKFold} on \texttt{match\_id}; mean over folds, lower is better for log-loss and Brier).}\label{tab:model-selection}
\small
...
\begin{figure}[H]
    ...
    \caption[xPass feature impact (SHAP bee-swarm)]{SHAP bee-swarm plot showing feature impact on the xPass model output. Positive SHAP values indicate that a feature pushes the prediction towards completion, while negative values push towards failure. Features are ordered by global importance, with the distance to the receiver (\texttt{min\_dist\_def\_to\_receiver}) and pass distance being the strongest drivers.}\label{fig:xpass-shap}
\end{figure}
...
The second term is the mirrored failure cost, and this mirroring is deliberate.
...
Each open-play pass is assigned an EPV value at its start and end location, read from a fixed EPV grid (a $32 \times 50$ lookup adapted from Friends-of-Tracking-Data~\cite{epv}, values ranging from ${\sim}0.004$ in the own goal area to ${\sim}0.57$ near the opponent goal) by bilinear interpolation (so value varies smoothly across the pitch rather than jumping between grid cells).
...
Because the index blends bold-geometry volume, high-value volume, their clean intersection, block-entry volume and per-pass boldness, a player can reach the top two different ways: as a high-volume metronome (Kroos, Modri\'{c}) or as a vertical specialist on lower volume whose few passes are bold and valuable (Alexander-Arnold).
...
\subsubsection[Internal Consistency (Cronbach's alpha)]{Internal Consistency (Cronbach's $\alpha$)}\label{sec:cronbach}

Cronbach's~$\alpha$~\cite{cronbach} asks whether the mother variables of a composite move together (whether they measure ``one thing'').
We report it per index in \cref{tab:alpha}, but we interpret it differently depending on what the index is meant to be.

\begin{table}[H]
\centering
\caption[Internal consistency (Cronbach's alpha)]{Internal consistency (Cronbach's $\alpha$, averaged over the 6 roles).}\label{tab:alpha}
\small
...
It is context-normalised by construction: the comparison lives inside each event, so the pitch-zone value level cancels. It is also bounded and easy to read on a single scale.
...
\begin{figure}[H]
    ...
    \caption[DQ Index vs.\ Value Impact]{Scatter plot of DQ Index (within-role percentile) against Value Impact (mean net value added vs.\ alternatives) grouped and coloured by macro role. The two metrics are strongly correlated ($\rho \approx 0.76$) by construction, showing that high relative choice accuracy translates directly to higher quantitative expected possession value.}\label{fig:dq-vs-value-impact}
\end{figure}

\subsection{Worked Example}\label{sec:dq-example}

The same Kroos event (Germany vs.\ Scotland, minute~16) is shown twice in \cref{fig:kroos-frame}: first, the eight in-frame candidates coloured by \textbf{xPass}; then the same eight coloured by \textbf{xEPV}, showing how the ranking changes.
...
\begin{figure}[H]
    ...
    \end{subfigure}
    \caption[The Kroos frame: xPass vs.\ xEPV reversal]{The Kroos frame (Germany vs.\ Scotland, min~16): xPass vs.\ xEPV ranking reversal.  On the left the eight in-frame candidates are coloured by completion probability (red~=~low, blue~=~high); on the right the same candidates are coloured by xEPV (red~=~negative net value, green~=~positive).  The ranking changes because xEPV multiplies probability by destination value and subtracts the mirrored failure cost.}\label{fig:kroos-frame}
\end{figure}
...
\end{itemize}
These four split the single Score into the two things a scout actually wants separated: how good the picks are (accuracy and avoid-worst rates) and how often they happen (elite\,/\,90 and avoid-poor\,/\,90 volumes); \cref{fig:dq-radar} shows the radar for Toni Kroos as a worked example.
...
Kimmich, Modri\'{c}, Xhaka and Barella all play deliberate tempo control, and xEPV is a per-pass quantity with no game state (no scoreline, no clock, no tactical instruction), so a deliberately safe, possession-securing pass reads as a weak selection.
...
This constraint does not spread evenly across the metric:
\begin{itemize}
...
Behind him Jorginho ($1.41$), Hjulmand ($1.08$) and Modri\'{c} ($0.92$) are the recognised tempo-setters of the tournament, and Kimmich ($0.85$), Wirtz ($0.84$) and Kroos ($0.82$) follow.
...
The role pools surface the right archetypes (\cref{fig:within-role-top5}; none need the index to be recognised, which is itself a face-validity check):
\begin{itemize}
...
H1, H2 and H3 each rate a player; together they leave him described by a rich, team-bias-adjusted feature vector. H4 stops rating and starts \emph{comparing}: it uses that vector to ask which players resemble each other.

\textbf{Hypothesis~4.}
With the three contextual metrics now in place, a player is already represented as a rich, team-bias-adjusted feature vector.
...
\begin{enumerate}
    ...
    \item \textbf{It separates players.} Best-match scores spread properly (median $79$, range $65$--$88$): $14$~players have a strong stylistic twin ($>85$) while $47$ are more one-of-a-kind ($<75$), exactly the shape a discriminating metric should produce. \Cref{fig:h4-top-pairs} shows the strongest within-role pairs in the pool.
\end{enumerate}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.92\textwidth]{images/top_pairs.png}
    \caption[Strongest within-role pairs]{The strongest within-role pairs in the pool, coloured by macro-role.  The scores top out near $88$ rather than at $100$, because two real footballers are never identical: a fixed maximum distance keeps the score absolute and comparable across pairs.  Every top pair is football-sensible (ball-playing centre-backs, attacking full-backs, elite wide dribblers), a direct face-validity check on the similarity metric.}\label{fig:h4-top-pairs}
\end{figure}
...
The strongest pairs in the pool are all sensible (John Stones~$\leftrightarrow$~Marc Gu\'{e}hi at $87.7$, Kyle Walker~$\leftrightarrow$~Di Lorenzo at $87.2$, Lamine Yamal~$\leftrightarrow$~Saka at $86.8$); \cref{fig:h4-dna-overlay} overlays the closest pair in each role, whose DNA polygons sit almost on top of each other.
...
We are grateful to our project advisor, Pol Mar\'{\i}n Mor\'{o}n, for his guidance and feedback throughout the development of this work. His input on the framing of the decision-quality metric, the off-ball movement study and the player-similarity approach helped shape the final form of the four studies.
% ============================================================
% REFERENCES
% ============================================================
...
\bibitem{xpasspaper}
Anzer, G., \& Bauer, P. (2022).
Expected passes: Determining the difficulty of a pass in football (soccer) using spatio-temporal data.
\emph{Data Mining and Knowledge Discovery}, \emph{36}(1), 295--317.
\url{https://doi.org/10.1007/s10618-021-00810-3}

\bibitem{cronbach}
Cronbach, L.\,J. (1951).
Coefficient alpha and the internal structure of tests.
\emph{Psychometrika}, \emph{16}(3), 297--334.
\url{https://doi.org/10.1007/BF02310555}

\bibitem{vaep}
Decroos, T., Bransen, L., Van Haaren, J., \& Davis, J. (2019).
Actions speak louder than goals: Valuing player actions in soccer.
In \emph{Proceedings of the 25th ACM SIGKDD International Conference on Knowledge Discovery \& Data Mining} (pp.\ 1851--1861). ACM.
\url{https://doi.org/10.1145/3292500.3330758}

\bibitem{availability}
Dick, U., Link, D., \& Brefeld, U. (2022).
Who can receive the pass? A computational model for quantifying availability in soccer.
\emph{Data Mining and Knowledge Discovery}, \emph{36}(3), 987--1014.
\url{https://doi.org/10.1007/s10618-022-00827-2}

\bibitem{wideopenspaces}
Fern\'{a}ndez, J., \& Bornn, L. (2018).
Wide open spaces: A statistical technique for measuring space creation in professional soccer.
In \emph{12th MIT Sloan Sports Analytics Conference}.

\bibitem{epvframework}
Fern\'{a}ndez, J., Bornn, L., \& Cervone, D. (2021).
A framework for the fine-grained evaluation of the instantaneous expected value of soccer possessions.
\emph{Machine Learning}, \emph{110}(6), 1389--1427.
...
In \emph{Advances in Neural Information Processing Systems} (Vol.\ 30, pp.\ 4765--4774). Curran Associates.

\bibitem{packing}
Rein, R., Raabe, D., \& Memmert, D. (2017).
``Which pass is better?'' Novel approaches to assess passing effectiveness in elite soccer.
\emph{Human Movement Science}, \emph{55}, 172--181.
\url{https://doi.org/10.1016/j.humov.2017.07.010}

\bibitem{epv}
Shaw, L. (2020).
\emph{Expected Possession Value (EPV) model and tutorials} [Computer software and tutorial series].
Friends of Tracking.
\url{https://github.com/Friends-of-Tracking-Data-FoTD/LaurieOnTracking}

\bibitem{xt}
Singh, K. (2019).
\emph{Introducing Expected Threat (xT)} [Blog post].
...
\bibitem{statsbomb}
StatsBomb. (2024).
\emph{StatsBomb Open Data: UEFA Euro 2024} [Data set].
\texttt{competition\_id\,=\,55}, \texttt{season\_id\,=\,282}; 51 matches.
\url{https://github.com/statsbomb/open-data}

\bibitem{transfermarkt}
Transfermarkt. (n.d.).
\emph{Market values for UEFA Euro 2024 participants} [Data set].
Retrieved 2024, from \url{https://www.transfermarkt.com/}
